...
\begin{figure}
	\centering
	\includegraphics[width=0.45\linewidth]{figs/voronoi}
	\caption[Example Voronoi diagram ]{An example Voronoi diagram for objects on a 2-dimensional space.  The black lines correspond to the borders of the Voronoi region, while the dashed lines correspond to the edges of the Delaunay Triangulation.}
	\label{fig:voro-ex}
\end{figure}
...
The heuristic is described in Algorithm \ref{alg:dgvh} in Chapter \ref{chapter:dgvh}.
...
The expected worst case cost of DGVH is \(O(\frac{\log^{3} n}{\log^{3} \log n} )\) \cite{dgvh}, regardless of the dimension \cite{dgvh}.\footnote{As mentioned in the previous footnote, if we are exchanging only short peers with a single neighbor rather than all our neighbors, the cost lowers to \(O(\frac{\log^{2} n}{\log^{2} \log n} )\).}
...
%\section{Remarks}
%
%UrDHT was a software implementation of many ideas we had when working on distributed hash tables.
%DGVH was implemented as part of UrDHT
%
%Unlike many DHTs, which leave fault tolerance as a problem for the implementations, UrDHT treats fault tolerance as an indispensable feature and it is crucial to specify exactly how it should work. 