...
\author{\IEEEauthorblockN{Andrew Rosen \qquad Benjamin Levin \qquad Anu G. Bourgeois}
	\IEEEauthorblockA{ Department of Computer Science \qquad Department of Computer Science \qquad \\ Temple University \qquad Temple University \qquad Georgia State University \\ Philadelphia, PA \qquad Philadelphia, PA \qquad Atlanta, GA \\ andrew.rosen@temple.edu \qquad blevin@temple.edu \qquad abourgeois@gsu.edu }
}
...
We have seen over the past decades with the development of cloud computing \cite{p2p-cloud}, volunteer computing frameworks like BOINC \cite{shvachko2010hadoop}.
...
One enormous benefit is a centralized system has much greater control in ensuring load-balancing, but this is not risks and issues specific to centralized approaches.
...
However, without a centralized framework, the network needs some kind of protocol to organize the various components in the network and cannot rely on a centralized coordinator.
...
Previous research has demonstrated the feasibility of a DHT-based distributed computational system.
...
We simulate our strategies on a Chord DHT \cite{chord},nodes obtain an ID, drawn from a cryptographic hash function, typically SHA1 \cite{sha1}.
