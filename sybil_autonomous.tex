...
Through these examples, we can see that multi-user distributed applications and content distribution systems are highly relevant.
Distributed Hash Tables (DHTs) are a class of well researched decentralized key-value storage systems that are most frequently used to construct the overlays of P2P file-sharing systems, such as the aforementioned BitTorrent \cite{bittorrent} and IPFS \cite{benet2014ipfs}.
...
One of the key components of a DHT is a cryptographic hash function, most commonly SHA-1.
...
Our strategies revolve around two key points.
First, we exploit the mechanisms DHTs use to handle churn, the constant change to the network topology caused nodes continually entering and leaving a distributed system.
This is combined with a variation of the Sybil attack, in which one node assumes multiple identities. 
...
%%% GRAB FROM CHORD REDUCE
%Update section
%expand Primary motivation
%The primayoutry motivation for us is creating a new viable type of platform for distributed computing.
...
%Lee et al.'s work \cite{leemap} demonstrated that a P2P network can be more than a way to distribute files and demonstrates how to accomplish different tasks using Map and Reduce functions over a P2P network.  
%Rather than using Chord, Lee et al. used Symphony \cite{symphony}, a DHT protocol.  
%To run a MapReduce job over the Symphony ring, a node is selected by the user to effectively act as the coordinator.  
%This ad-hoc coordinator then performs a bounded broadcast over a subsection the ring.  
%Each node repeats this broadcast over a subsection of that subsection, resulting in a tree with the first node at the top.  Map tasks are disseminated evenly throughout the tree and their results are reduced on the way back up to the ad-hoc master node.  This allows the ring to disseminate Map and Reduce tasks without the need for a coordinator responsible for distributing these tasks and keeping track of them, unlike Hadoop.
%
...
%However, without a centralized organizer, the network needs some kind of protocol to coordinate the various parts in the network.
...
We call these virtual nodes Sybils, as this duplication is almost identical to the way the Sybil attack  \cite{sybil} creates virtual nodes in a P2P system.
...
\begin{table}
	\centering
	\caption{The median distribution of tasks (or files) among nodes.  We can see the standard deviation is fairly close to the expected mean workload ($\frac{tasks}{nodes}$). Each row is the average of 100 trials.  Experiments show little deviation in the median load of the network.}
...
%TODO add zoomed in view : ANDREW
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.9\linewidth]{figs/workloadDistribution}
	\caption[Workload Distribution in a DHT]{The probability distribution of workload in a DHT with 1000 nodes and 1,000,000 tasks or files.  The vertical dashed line designates the median. The inset rectangle is an enlarged view of extreme values of the x axis.  Keys were generated by feeding random numbers into the SHA-1 hash function, a favorite for many distributed hash tables.}
...
%The most blatant example of this is pair of nodes located at approximately $(.75, .5)$; the bottom node of this pair receives only a single task.
...
\begin{figure}
	...
	\caption[Distribution of Nodes and Tasks in a Chord DHT]{A visual example of data and nodes in a Chord DHT with 10 nodes (red circles) and 100 tasks (blue pluses). Nodes are responsible for all tasks that are along the perimeter between themselves and their predecessor (closest node counterclockwise).}
	\label{fig:exampleChordDistribution}
\end{figure}

Figure \ref{fig:exampleChordEvenDistribution} shows the same network, but the nodes are evenly distributed, rather than generated by the SHA-1 hash function.
...
\begin{figure}
	...
	\caption[Distribution of Tasks in a Chord DHT  with Evenly Distributed Nodes]{A visual example of data and nodes in a Chord DHT with 10 evenly distributed nodes (red circles) and 100 tasks (blue pluses).}
...
We examined four different strategies for nodes to perform autonomous load balancing.
...
We discuss the effectiveness of each strategy and present the  simulation results in Section \ref{sec:autonomous-results}.
...
%Ask about ^^ this
It is possible for an invitation to be refused if no predecessor is at the \texttt{sybilThreshold} or each predecessor has too many Sybils.
...
Nodes obtain an ID, drawn from a call to SHA-1.
...
\subsubsection*{IDs and Keys} We will be using SHA-1, a 160-bit hash function, to generate node IDs and keys for tasks.  
...
\subsubsection*{Max Sybils} \texttt{maxSybils} is the maximum Sybils a node can have at once.  We tested 5, the default, and 10.
...
%	\begin{figure}[h]
%		\centering
%		\includegraphics[width=0.9\linewidth]{figs/churnVsTime}
%		\caption[Churn vs Runtime factor]{Effect of churn has on runtime.}
...
%	\begin{figure}
%		\centering
%		\includegraphics[width=0.9\linewidth]{figs/churnVsWork}
%		\caption[Churn vs average work per tick]{Effect of churn on average workload.}
...
%	
	...
	\begin{figure}
		...
		\caption[Random injection vs no strategy after 5 ticks.]{After 5 ticks, the network using random injection has significantly less underutilized nodes.}
	...
	\begin{figure}
		...
		\caption[Random injection vs no strategy after 35 ticks.]{After 35 ticks, the network using random injection has significantly less underutilized nodes and substantially more notes with some or lots of work.}
	...
	\begin{figure}
		...
		\caption[Churn vs Random Injection after 35 ticks.]{After 35 ticks, the network using random injection load-balances significantly better than the network using Churn.}
...
%	\begin{figure}
%		\centering
%		\includegraphics[width=0.9\linewidth]{figs/inviteStableHist5}
...
%	\end{figure}
	...
	\begin{figure}
		...
		\caption[Invitation vs no strategy after 35 ticks]{At 35 ticks, we can see the network using the invitation strategy perform markedly better than the network using no strategy. The highest load is around 500 tasks in the network using invitation, compared to approximately 650 tasks in the network using no strategy.}
...
%	
	...
	\begin{figure}
		...
		\caption[Invitation vs smart neighbor injection after 35 ticks.]{After 35 ticks, we see the invitation strategy has significantly less nodes with a small workload and many more nodes with large workloads.}
	\end{figure}
